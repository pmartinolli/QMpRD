...
-   Un aide-mémoire surnommé le « **Foutaisomètre** », recense l'essentiel des questions à se poser pour une publication de recherche <https://github.com/pmartinolli/TM-bullshitometer>.

###  Distinguer les sources 

####  Savoir décrypter une référence bibliographique[]{#id-ref-biblio label="id-ref-biblio"}
...
-   Un tableau des disciplines UdeM et des styles bibliographiques habituellement recommandés <https://bib.umontreal.ca/citer/styles-bibliographiques/choisir-son-style>.
...
-   Les **logiciels bibliographiques** comme Zotero et Endnote automatisent la création de bibliographies et de citations abrégées et vous font gagner un temps précieux et aussi un temps de qualité Voir chapitre p. .
...
-   [Récupérez les références de ces documents pertinents et leurs fichiers PDF dans Zotero.☐]{style="color: purple"}

-   [Optionnel: dans Zotero, ajoutez le marqueur ]{style="color: purple"}[`_bibliographieAnalysée`]{style="color: purple"}[ à la référence contenant la bibliographie que vous venez d'analyser et de parcourir. Colorez le marqueur si besoin]{style="color: purple"} [voir p. ]{style="color: purple"}[. ☐]{style="color: purple"}

####  Sources primaire, secondaire ou tertiaire
...
-   Ensemble de **données brutes** sur un sujet.

-   Un matériau de première main d'un phénomène : statistiques, données d'enquête, résultats bruts d'une étude, journal intime, carnet de notes personnelles, discours, etc.

-   Permet de mieux **vérifier**.
...
-   Une **sélection** et une **compilation** synthétique de sources primaires et de sources secondaires.
...
-   L'auteur est-il **seul** ou sont-ils **plusieurs** ?
    ...
    -   L'**ordre de citation** est signifiant (à moins d'indication contraire, mais c'est rare) : le premier auteur a plus contribué que le dernier.
        ...
        -   Conseil : pour vos propres recherches, dès votre première réunion d'équipe, déterminez l'ordre de citation des auteurs <https://www.science.org/content/article/i-avoided-authorship-discussions-collaborators-until-i-learned-some-hard-lessons>, leurs rôles et leurs contributions.
...
-   Les types de contribution de chacun sont-elles précisées ? Voir la **taxonomie CRediT** qui permet de détailler les rôles de chacun <https://coop-ist.cirad.fr/etre-auteur/reconnaitre-tous-les-contributeurs/3-la-taxonomie-credit-pour-identifier-toutes-les-contributions>.
...
-   C'est robuste mais cela ne détecte pas bien la **fraude**.
    ...
    -   Un article publié dont on se rend compte plus tard qu'il est une fraude ou qu'il contient des erreurs car publié trop vite est retiré. On dit qu'il est **rétracté** voir p. **.**
...
-   Les **revues prédatrices** prétendent faussement faire un travail de ce type voir p. .

##### Comment vérifier qu'une revue à un comité de lecture valide : 

-   [Ulrich's Periodical Directory]{style="color: purple"} [<http://ulrichsweb.serialssolutions.com/login>]{style="color: purple"}[.. Si vous voyez un habit rayé d'arbitre de sport (refereed) alors la revue possède un processus de révision par les pairs.]{style="color: purple"}

-   [Liste sélective des revues indexées par Web of Science (]{style="color: purple"}*[Master Journal List]{style="color: purple"}*[]{style="color: purple"} [<https://mjl.clarivate.com/home>]{style="color: purple"}[.) ont toutes un processus de révision par les pairs.]{style="color: purple"}

-   [Sélection dans le menu de gauche des pages de Résultats de certaines bases de données bibliographiques (appelées différemment selon les bases).]{style="color: purple"}

    -   [Sofia : appellation « Publication scientifique »]{style="color: purple"}

    -   [ProQuest : « Revue par les pairs »]{style="color: purple"}

    -   [Erudit : « Revue savante » + icone ronde noire cochée.]{style="color: purple"}

    -   [EBSCO : « peer reviewed » ou « Revues académiques (relues par un comité de lecture) » ou « Revues universitaires » (cela dépend de la base de données). ]{style="color: purple"}
...
-   Le document est-il **cité** par d'autres sources ?
    ...
    -   Recommandations :
        ...
        -   Installer le plugin Scite pour Zotero <https://github.com/scitedotai/scite-zotero-plugin/releases>. Une fois installé, ajouter les colonnes *Mentioning, Contrasting, Supporting* et/ou *Total Distinct Citing Publications*.

        -   Installer le plugin Scite pour navigateur internet <https://scite.ai/extension-install>.

-   Le document a-t-il été **rétracté** voir p. ?

-   Le document est-il **critiqué** (ou parfois loué) sur **PubPeer** <https://pubpeer.com/> ?

    -   Recommandations :

        -   Installer l'extension PubPeer pour Zotero <https://github.com/PubPeerFoundation/pubpeer_zotero_plugin/releases/latest>. Une fois installé, ajouter la colonne PubPeer.

        -   Installer l'extension PubPeer pour Firefox, Chrome, Edge ou Safari <https://pubpeer.com/static/extensions>.

        -   Installer les extentions Scite pour Zotero et navigateur (voir plus haut). Scite utilise désormais son propre système de détection de rétractions.
