...
-   Comment ?
    -   Allez explorer les thésaurus de certaines bases de données :*MEDLINE, PsychInfo, Sociological Abstracts,*etc.
        -   Pour les catalogues d'institution québécoises, le thésaurus est souvent le Répertoire des vedettes-matières de l'Université Laval (RVM) <https://rvmweb.bibl.ulaval.ca/>. Je suis allé le parcourir : ☐
...
-   Faites un thésaurus basé sur vos objectifs de recherche et votre corpus, pas sur l'organisation théorique et complète des informations. Un thésaurus est toujours **adapté** à un projet.
    ...
    -   Exemple : si votre projet ne traite que de sports aquatiques, il vaut mieux avoir $\begin{array}{cc}
        natation\\
        waterpolo\\
        plongeon\\
        autre
        \end{array}$` `plutôt que $\begin{array}{cc}
        natation\\
        waterpolo\\
        plongeon\\
        course\\
...
-   Astuce de la « sous-catégorie obèse » : fusionnez une sous-catégorie trop volumineuse (par rapport à toutes les autres qui sont rachitiques) avec la catégorie supérieure.
    ...
    -   Exemple : Avant (bof) : $\begin{array}{clc}
        \\
        LIS &  & (n=52)\\
         & Archive & (n=6)\\
         & Biblioth\grave{e}que & (n=32)\\
         & Citation & (n=4)\\
         & Textmining & (n=3)\\
        \\
        \end{array}$ -->Après (mieux) : $\begin{array}{clc}
        Biblioth\grave{e}que &  & (n=52)\\
         & Archive & (n=6)\\
         & Citation & (n=4)\\
         & Textmining & (n=3)
        \end{array}$
...
-   [Dans une notice : onglet Marqueurs \> Ajouter \> rédiger un nouveau marqueur.]{style="color: purple"}

-   [☐ Astuce : faire précéder chaque marqueur unique du ]{style="color: purple"}**[caractère \_]{style="color: purple"}**

    -   [Ils sont signés comme étant vos marqueurs à vous, issus d'une précieuse liste contrôlée.]{style="color: purple"}

    -   [Ils apparaissent en haut des listes.]{style="color: purple"}

-   [☐ Astuce : faire précéder vos marqueurs temporaires ou de « cuisine interne » (réorganisation, hésitation, archives, etc.) avec le ]{style="color: purple"}**[caractère =]{style="color: purple"}**

    -   [Ils sont signés aussi, et ils ne se mélangent pas à votre liste contrôlée de \_marqueurs.]{style="color: purple"}

    -   [Ils apparaissent en groupe, juste après les \_marqueurs.]{style="color: purple"}

-   [☐ Astuce : vous pouvez colorer avec 9 couleurs des marqueurs qui seront fixés de manière permanente dans le nuage des marqueurs à droite.]{style="color: purple"}

    -   [Nuage des marqueurs \> clic-droit sur un marqueur \> Attribuer une couleur.]{style="color: purple"}

    -   [Suggestion de marqueurs à colorer : ]{style="color: purple"}[`_nettoyé`]{style="color: purple"}[,]{style="color: purple"}[` _publication majeure`]{style="color: purple"}[,]{style="color: purple"}[` _lu`]{style="color: purple"}[.]{style="color: purple"}

#### Création du thésaurus personnel dans un tableau 

-   [☐ Ouvrir un fichier Word ou LibreOffice :]{style="color: purple"}

    -   [☐ Faire tout tenir sur une page recto : Réduire les marges et haut et bas de page. Créer 2 colonnes. ]{style="color: purple"}

    -   [☐ Une fois terminé, imprimer cette page et la coller comme couverture de votre cahier de recherche. Ainsi, vous l'aurez facilement toujours sous les yeux. ]{style="color: purple"}

-   [☐ Faire 2 ou 3 colonnes : ]{style="color: purple"}

    -   [2 colonnes = une colonne pour les marqueurs uniques, une colonne pour les renvois.]{style="color: purple"}

    -   [3 colonnes = deux colonnes pour les marqueurs uniques (organisés sur deux niveaux), une colonne pour les renvois.]{style="color: purple"}

        -   [Voir Exemple ci-dessous.]{style="color: purple"}

-   [☐ Lister tous les marqueurs uniques. \[3h+\]]{style="color: purple"}

    -   [Choisir une règle de forme unique ]{style="color: purple"}

        -   [Exemple : en français, au singulier et débutant avec une majuscule après le \_ ou le =. ]{style="color: purple"}

        -   [Exemple : en anglais, au singulier et tout en minuscule (mon préféré).]{style="color: purple"}

        -   [Car dans Zotero les marqueurs sont sensibles à la casse et aux accents. ]{style="color: purple"}

    -   [☐ Parcourir sa collection de références et repérer les thèmes. Si un nouveau thème est pertinent, créez un marqueur.]{style="color: purple"}

-   [☐ Regrouper les marqueurs par grands thèmes (niveau 1). ]{style="color: purple"}

    -   [Si on veut, on peut donner un niveau supplémentaire pour nicher certains sous-thèmes (niveau 2) dans des grands thèmes (niveau 1).]{style="color: purple"}

-   [☐ Lister tous les renvois, alias, variants, thèmes proches ou thèmes plus spécifiques pertinents dans la colonne des renvois. \[2h+\]]{style="color: purple"}
...
-   [On arrive sur la recherche avancée avec une requête cochée sur Keyword. Il faut donc : ]{style="color: purple"}

    -   [Chercher d'abord un Keyword à la fois et ensuite combiner les historiques de recherche.]{style="color: purple"}

    [OU ]{style="color: purple"}

    -   [Lancer une recherche avec plusieurs Keywords mais séparés par un ]{style="color: purple"}[`AND`]{style="color: purple"}[ explicite. ]{style="color: purple"}

        -   [En effet, c'est une des rares base de données qui ne remplace pas les espaces entre les mots par des ]{style="color: purple"}[`AND`]{style="color: purple"}[. Par défaut, des mots séparés par des espaces sont cherchés comme un groupe de mots entre guillemets.]{style="color: purple"}

### Option de recherche avancée 

-   [Ouvrir le ]{style="color: purple"}*[Term Finder ]{style="color: purple"}*[(bouton vert). ]{style="color: purple"}

-   [Chercher un terme, le cocher puis et cliquer sur ]{style="color: purple"}*[Add to Search Options]{style="color: purple"}*[. ]{style="color: purple"}

    -   [Les termes MeSH ont une hiérarchie avancée : des termes plus génériques et des termes plus spécifiques. ]{style="color: purple"}

        -   [Cocher]{style="color: purple"}*[ Explode]{style="color: purple"}*[ pour chercher un niveau général et tous les niveaux plus spécifiques.]{style="color: purple"}

    -   [Cocher ]{style="color: purple"}*[Focus ]{style="color: purple"}*[pour chercher seulement les sujets qui ont été indexés comme sujets majeurs du document (ceux précédés d'une astérisque).]{style="color: purple"}

-   [Recommencer pour chaque terme puis combiner les recherches pertinentes.]{style="color: purple"}
