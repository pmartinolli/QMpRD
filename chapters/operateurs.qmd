...
-   Pourquoi ? Pour aider les moteurs de recherche à combiner les mots entre eux. On dit aussi « opérateur booléen. »
...
-   **C'est l'opérateur logique le plus important.**
...
-   Pourquoi ? Parce que dans la plupart des bases de données universitaires, si vous saisissez un mot le moteur de recherche ne va chercher que ce mot **exactement**.
...
-   [Comment ?]{style="color: purple"}
    ...
    -   [**Ajouter à un endroit stratégique vers la fin du mot une astérisque\* collée au mot.**]{style="color: purple"}

        -   [L'astérisque remplace aucune lettre ou une série variable de lettres.]{style="color: purple"}

        -   [Idéal pour repérer les féminins, les pluriels et les déclinaisons proches.]{style="color: purple"}

    -   [Exemples :]{style="color: purple"}

        -   [``` dementia elder``*`` board game``* ```]{style="color: purple"}

        -   [``` play``*`` kid``*`` memor``* ```]{style="color: purple"}
...
-   [Comment ?]{style="color: purple"}
    ...
    -   [Dans une parenthèse, écrire tous les mots-clés et les séparer par OR (espace OR espace).]{style="color: purple"}

    -   [Exemples :]{style="color: purple"}

        -   [``` (dementia OR cogniti``*``) AND (elder``*`` OR aged) AND (board OR card OR role-play``*``) AND game``* ```]{style="color: purple"}
...
-   Pourquoi ? Permet d'avoir **moins de résultats mais toujours pertinents**, ou bien de présenter les meilleurs résultats en premier.

    -   En effet, mettre un groupe de mots entre guillemets carrées va chercher exactement ces mots dans l'ordre indiqué. S'il n'y a pas de guillemets, la machine va opérer un AND entre les mots (ce qui va chercher tous les mots mais n'importe où dans le document, donc quelques documents pertinents mais aussi un grand nombre de documents pas pertinents).

-   [Comment ?]{style="color: purple"}

    -   [Utiliser les guillemets carrés de votre clavier (clavier canadien ou américain : avec la touche 2).]{style="color: purple"}

    -   [Ne pas utiliser les chevrons « » (guillemets triangulaires), ne pas utiliser les doubles apostrophes ' ' : cela ne fonctionne pas.]{style="color: purple"}

    -   [Choisir l'expression exacte à recherche.]{style="color: purple"}

        -   [Dans les bases de données universitaires, il est recommandé de commencer la recherche SANS les guillemets. Ensuite, s'il y a trop de résultats, on peut ajouter des guillemets si c'est applicable.]{style="color: purple"}
...
-   Pourquoi ? Dans le cas où on a trop de résultats et que l'on remarque que beaucoup de résultats correspondent à un critère qui ne nous intéresse pas. On souhaite alors **enlever automatiquement** de la liste de résultats tous les documents qui correspondent à un ou plusieurs critères.

-   [Comment ?]{style="color: purple"}

    -   [Rédiger sa requête et, à la fin, ajouter l'opérateur NOT suivi du mot qui va éliminer tous les documents utilisant ce mot.]{style="color: purple"}

    -   [On peut supprimer plusieurs mots en ajoutant`NOT (mot1 OR mot2 OR mot3)`par exemple.]{style="color: purple"}

-   [Exemples d'utilisation :]{style="color: purple"}

    -   [Une recherche est envahie par des résultats d'un domaine complètement différent.]{style="color: purple"}

        -   [Exemple :``` senateur``*`` Ottawa ```va donner des résultats de sport (puisqu'une équipe de hockey s'appelle les Sénateurs d'Ottawa) et des résultats de politique. Si on veut seulement des résultats de politiques, il faut utiliser`senateur Ottawa NOT hockey.`]{style="color: purple"}

    -   [On peut utiliser le NOT pour enlever des publications gouvernementales d'une liste de résultats.]{style="color: purple"}

        -   [Exemple :``` mon sujet NOT (gouvernement``*`` OR minist``*``) ```]{style="color: purple"}

    -   [On peut utiliser le NOT pour enlever des documents d'une discipline qui ne nous intéresse pas.]{style="color: purple"}

        -   [Exemple :``` drogue``*`` NOT (pharma``*`` OR biochim``*``) ```]{style="color: purple"}
