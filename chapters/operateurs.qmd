...
-   Ce que font ces boîtes de dialogue, c'est :
    ...
    3.  Calculer quels sont les concepts proches
        -   pas avec des `OR` ou des troncatures
        -   avec un chiffrement des mots-clés sous la forme de vecteurs (des valeurs numériques)
    4.  Croiser ces concepts et appliquer la recherche sur un corpus de références.
-   C'est pratique pour les déburants qui ne savent pas utiliser les opérateurs mais :
    -   C'est plutôt **limité en terme de puissance** de recherche. En effet, il est rare que le nombre de résultats dépasse dix ou quelques dizaines d'études.
    -   On a **peu de contrôle** sur les recherches soumises et sur le processus d'essais-erreurs-corrections. Il est difficile de savoir ce qu'il se passe à chaque intéraction.
